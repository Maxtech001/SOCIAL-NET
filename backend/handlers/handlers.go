...
func FollowRequestHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		if request.Accept {
			...
			err := database.UpdateRelationship(1, user.Id, request.TargetId)
			if err != nil {
...
func FollowHandler(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodPost:
		...
		if existingRelationship != nil {
			...
			CustomResponse(w, 204, "Unfollowed successfully")
		...
		err = CreateNotification(request.TargetId, "/profile", "You have a new follower request!")
		if err != nil {
...
func SearchUserHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		searchUsers, err := database.SearchUser(nameParts)
		if err != nil {
			fmt.Println(err)
			ServerError(w, err.Error())
			return
...
func FeedHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		posts, err := database.GetPostsCustomQuery("SELECT P.*, U.firstname, U.lastname FROM posts as P INNER JOIN users as U ON P.creator_id = U.id WHERE privacy != 0 ORDER BY id DESC LIMIT ? OFFSET ? ", maxPerPage, offset)
		...
		for _, post := range posts {
			...
			if post.Privacy == 1 {
				...
				if postMeta != nil {
					//convert the allowedViewers to json object
					var allowedViewers = make([]int, 0)
					err := json.Unmarshal([]byte(postMeta.AllowedViewers), &allowedViewers)
					if err != nil {
						fmt.Println("Error with unmarshaling allowedViewers", err.Error())
						continue
					}
					for _, allowedUser := range allowedViewers {
