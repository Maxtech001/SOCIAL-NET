...
func SearchUser(nameParts []string) ([]User, error) {
...
func GetPostMetadata(postId int) (*PostMetadata, error) {
	...
	return QueryItem[PostMetadata]("SELECT allowedViewers FROM post_meta WHERE postId=?", func(row *sql.Row, item *PostMetadata) error {
		return row.Scan(&metadata.AllowedViewers)
	}, postId)
}
...
func GetRelationshipsByFollowingID(id int) ([]Relationship, error) {
	return QueryItems[Relationship]("SELECT * FROM relationships WHERE following_id=?", func(rows *sql.Rows, relation *Relationship) error {
		return rows.Scan(
			&relation.FollowerId, &relation.FollowingId, &relation.Timestamp)
	}, id)
}

// Modified to include names and profile pic
func GetRelationshipRequestsByFollowingId(id int) ([]Relationship, error) {
	return QueryItems[Relationship]("SELECT R.*, u.firstname, u.lastname, u.avatar_path FROM relationships AS R INNER JOIN main.users u on R.following_id = u.id WHERE follower_id=? AND accepted = 0", func(rows *sql.Rows, relation *Relationship) error {
		return rows.Scan(
...
func GetRelationshipsByFollowerID(id int) ([]Relationship, error) {
	return QueryItems[Relationship]("SELECT * FROM relationships WHERE follower_id=?", func(rows *sql.Rows, relation *Relationship) error {
		return rows.Scan(
			&relation.FollowerId, &relation.FollowingId, &relation.Timestamp, &relation.Accepted)
	}, id)
}
